...
def find_10_most_common_names(text):
    ...
    for word in words:
        ...
        if word == word.title():
            if previous_word in ['Mr.', 'Mrs.', 'Mme.',
                                 'Ms.'] or previous_word[-1] != '.':
                # if a non-existant key is accessed then its value is zero -
                # if the key does exist the dictionary behaves as normal
    ...
    # somewhere to save the most common names and their counts
    output_dic = {}
    # repeat 10 times, i isn't used for anything
    for i in range(10):
        # before checking the counter dictionary we set the maximum count to
        # zero
        max_value = 0
        # and the key for maximum count == 0 is an empty string
        max_key = ''
        # get all key-value pairs, one at a time
        for key, value in counter.items():
            # if the value of the current key-value pair is greater than the
            # current max count then...
            if value > max_value:
                # override the maximum count with the current count
                max_value = value
                # and save the key for this value
                max_key = key
        # once you've looped through all dictionary items put key-value pair
        # with the highest count value in the dictionary for output
        output_dic[max_key] = max_value
        # and remove it from the counter dictionary - otherwise we would get
        # the same key-value pair again
        del counter[max_key]
...
if __name__ == '__main__':
    # this will give different results each time you run the code 'cause
    # dictionaries
    print(find_10_most_common_names(text))
